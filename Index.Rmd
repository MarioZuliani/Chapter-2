...
###Predictions
a. Increasing shrub density from shrub-free locations to relatively higher densities positively predict animal community density; b. Increasing shrub cover at varying densities positively predicts animal community densities; 
...
ggplot(final.data, aes(density, animals, color = microsite)) +
  geom_point() +
  geom_errorbar(aes(ymin=animals-se_animal, ymax=animals+se_animal), width=.2,
...
ggplot(final.data, aes(density, richness, color = microsite)) +
  geom_point() +
  geom_errorbar(aes(ymin=richness-se_richness, ymax=richness+se_richness), width=.2,
                 position=position_dodge(0.05)) +
  facet_wrap(~phylum) +
  scale_x_continuous(breaks = c(0,2,4,6,8,10,12)) +
  scale_color_brewer(palette = "Set1") + theme_classic() +
...
ggplot(final.data, aes(density, Simpson, color = microsite)) +
  geom_point() +
  geom_errorbar(aes(ymin=Simpson-se_simpson, ymax=Simpson+se_simpson), width=.2,
                 position=position_dodge(0.05)) +
  facet_wrap(~phylum) +
  scale_color_brewer(palette = "Set1") + theme_classic() +
  geom_smooth(method = lm, se = FALSE) + 
...
carrizo4 <- get_map(location = c(lon = -119.6287, lat = 35.11975), zoom = 18, maptype = "satellite")
carrizo4
site.wide.map <- ggmap(carrizo4)
site.wide.map <- site.wide.map +
  geom_point(data=site.4, aes(x=shrub.lng, y=shrub.lat, color = Site), alpha = 3/10, size =4) +
...
carrizo3 <- get_map(location = c(lon = -119.67212, lat = 35.16175), zoom = 18, maptype = "satellite")
carrizo3
site.wide.map <- ggmap(carrizo3)
site.wide.map <- site.wide.map +
  geom_point(data=site.3, aes(x=shrub.lng, y=shrub.lat, color = Site), alpha = 3/10, size =4) +
  labs(title = "Site Wide Species Observations", x = "longitude", y = "latitude")
site.wide.map
```
...
ggplot(final.data, aes(max_temp, animals, color = microsite)) +
  ...
  scale_color_brewer(palette = "Set1") + theme_classic() +
  geom_smooth(method = lm, se = FALSE) + 
  labs(x = "Mean Temperature", y = "animal density")
```
```{r}
ggplot(density_simple2, aes(cover, animals)) +
