...
library(tidyverse)
library(vegan)
finalvegan <- data.final %>%
  group_by(species, phylum, density, daily_animal_totals) %>%
  summarise(captures = sum(daily_animal_totals))

finalvegan$species <- gsub(" ", "", finalvegan$species)
finalvegan$species <- as.factor(finalvegan$species)
finalvegan$uniID <- paste(finalvegan$species, finalvegan$density)
str(finalvegan)

library(dplyr)
finalvegan <- finalvegan %>% 
  ungroup()%>%
  dplyr::select(species, daily_animal_totals)
finalvegan <- finalvegan %>% group_by(species, daily_animal_totals) %>% summarise(captures = sum(daily_animal_totals))

community <- finalvegan %>% spread(species, captures)
community[is.na(community)] <- 0

community <- community %>%
  ungroup() %>%
  dplyr::select(-daily_animal_totals)
```
