...
anova(d1, test = "Chisq")
de1 <- emmeans(d1, pairwise~ method | phylum, var = "density")
de1
```
```{r}
...
```{r}
d9<- glm(RDM ~ density*microsite*phylum*richness, family = "quasipoisson", data = final.data)
anova(d9, test = "Chisq")
...
```{r}
d10<- glm(RDM ~ density*microsite*phylum*Evenness, family = "quasipoisson", data = final.data)
...
rdmevenness <- ggplot(final.data, aes(RDM, Evenness, color = microsite)) +
  geom_point(size = 0.5) +
  facet_wrap(~phylum) +
  scale_color_brewer(palette = "Set1") + theme_classic() + theme(legend.position = "none") +
