...
```{r}
masterphoto <- photo %>%
  group_by(site, plot,  `level (l,m,h,none)`, density_level,Measure, date, day_sample, RTU) %>%
...
mastertransect <- transect %>%
  group_by(Site.Number, plot, Site.Density, density_level, Measure, Date, day_sample, RTM) %>%
  summarise(daily_animal_totals = sum(animal.hit))
...
```{r}
masterfocal <- focal %>%
  group_by(Site, plot, site.density, density_level, Measure, Date, day_sample, RTU) %>%
  summarise(daily_animal_totals = sum(animal.hit))
