...
excludes <- c("none", "None", "unidentifiable", "unkown", "", "Unidentifiable")
...
##Stats
```{r}
m <- glm(captures~level..l.m.h.none., data = level)
summary(m)
```
```{r}
...
```{r}
mp <- glm(rate~level..l.m.h.none., data = level)
summary(mp)
```
```{r}
anova(mp)
...
ggplot(transect.level,  aes(Site.Density, captures)) +
  geom_boxplot() +
  labs(y = "capture rate", x = "site density")
```
