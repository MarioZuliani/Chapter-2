...
```{r}
photopresence <- ggplot(level, aes(dense.level, captures, fill = as.character(site))) +
  geom_boxplot() +
...
summary(mg)
anova(mg, test = "Chisq")
emmeans(mg, pairwise~dense.level)
```
...
```{r}
anova1 <- anova(captures~dense.level, data = level)
```

```{r}
fig1 <- ggplot(level, aes(dense.level, captures)) +
  geom_boxplot() +
