...
photovegan$uniID <- paste(photovegan$site, photovegan$dense.level)
str(photovegan)
```
...
```{r}
ggplot(shortvegan, aes(site.density, Species)) + geom_boxplot() + ylab("Species Diversity")
ggplot(shortvegan, aes(site.density, Even)) + geom_boxplot() + ylab("Species Evenness")
ggplot(shortvegan, aes(site.density, H)) + geom_boxplot() + ylab("Species Richness")
```
```{r}
...
```{r}
ggplot(shorttransect, aes(site.density, Species)) + geom_boxplot() + ylab("Number of Species")
ggplot(shorttransect, aes(site.density, Even)) + geom_boxplot() + ylab("Species Evenness")
ggplot(shorttransect, aes(site.density, H)) + geom_boxplot() + ylab("Species Richness")
```
```{r}
...
```{r}
ggplot(shortfocal, aes(site.density, Species)) + geom_boxplot() + ylab("Number of Species")
ggplot(shortfocal, aes(site.density, Even)) + geom_boxplot() + ylab("Species Evenness")
ggplot(shortfocal, aes(site.density, H)) + geom_boxplot() + ylab("Species Richness")
```
```{r}
...
Questions:
1) When running the stats for diversity do i just look at the H value or should i be considering the evenness for the stats?
2) Are all p-values significant or not and done properly for reporting
3) For final figure putting 9 figures into 1 with ggarrange may look bad and clustered. Perhaps seperating them into groups of 3?