...
masterdata2 <- masterdata %>%
  group_by(date) %>%
  summarize(richness = n_distinct(species), count = sum(daily_animal_totals))
masterdata2$date <- NULL

masterdata3 <- masterdata

masterdatarichness <- merge(masterdata3, masterdata2, all=TRUE)
masterdatarichness$count <- NULL
names(masterdatarichness)[10] <- "daily_animal_richness"

write.csv(masterdatarichness, "data_2019_compiled1.csv")
```
...
m1 <- glm(animals~density_levels, family = "quasipoisson", data = density_simple)
anova(m1, test = "Chisq")
emmeans(m1, pairwise~density_levels)
```
```{r}
...
```
```{r}
m2 <- glm(richness~density_levels, family = "quasipoisson", data = density_simple)
anova(m2, test = "Chisq")
emmeans(m2, pairwise~density_levels)
```


```{r}
ggplot(density_simple, aes(as.character(density), animals, color = density_category)) +
  geom_boxplot() +
  scale_color_brewer(palette = "Set1") +
  labs(x = "shrub density", y = "total animal density")

m3 <- glm(animals~density, family = "quasipoisson", data = density_simple)
anova(m3, test = "Chisq")
emmeans(m3, pairwise~density)
```
```{r}
...
ggplot(data_simple, aes(density_levels, animals)) +
  geom_boxplot() +
  scale_color_brewer(palette = "Set1") +
  labs(x = "shrub density level", y = "animal abundance")

m5 <- glm(animals~density_levels, family = "quasipoisson", data = data_simple)
anova(m5, test = "Chisq")
emmeans(m5, pairwise~density_levels)
```
```{r}
...
m6 <- glm(animals~density_levels, family = "quasipoisson", data = data_simple)
anova(m6, test = "Chisq")
